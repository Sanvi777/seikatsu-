...
export default function LoginPage() {
  ...
  const handle = async()=>{
    ...
    if(mode==='signup'){ setMessage('Account created! Please check your email to confirm. 🎌'); return; }
    router.push('/');
  };
    ...
    <div style={{minHeight:'100vh',background:'#050508',display:'flex',fontFamily:"'DM Sans',system-ui,sans-serif",overflow:'hidden',position:'relative'}}>
      ...
      <div style={{position:'fixed',inset:0,pointerEvents:'none',zIndex:1,overflow:'hidden'}}>
        {Array.from({length:40},(_,i)=>(
          <div key={i} style={{
            ...
            width:'1px',
            height:`${15+(i*3)%20}px`,
            background:`rgba(100,150,255,${0.1+(i*0.01)%0.2})`,
            animation:`rain ${1.5+(i*0.1)%2}s ${(i*0.15)%3}s infinite linear`,
          }}/>
        ))}
      </div>
      ...
      <div style={{position:'fixed',inset:0,pointerEvents:'none',zIndex:1,overflow:'hidden'}}>
        {[
          ...
          <div key={i} style={{
            ...
            fontSize:k.size,color:'rgba(255,100,50,0.06)',
            fontWeight:'900',lineHeight:1,
            animation:`float ${k.dur} ${k.delay} infinite ease-in-out`,
            fontFamily:'serif',
          }}>{k.char}</div>
        ))}
      </div>
      ...
      <div style={{position:'fixed',bottom:0,left:'50%',transform:'translateX(-50%)',pointerEvents:'none',zIndex:1}}>
        <svg width="300" height="200" viewBox="0 0 300 200" fill="none">
          ...
          <rect x="30" y="30" width="8" height="20" fill="rgba(255,60,0,0.06)"/>
          <rect x="262" y="30" width="8" height="20" fill="rgba(255,60,0,0.06)"/>
        </svg>
      </div>
      ...
      <div style={{position:'fixed',bottom:0,left:'30%',width:'40%',height:'3px',background:'rgba(255,60,0,0.15)',boxShadow:'0 0 30px rgba(255,60,0,0.3)',pointerEvents:'none',zIndex:2,borderRadius:'50%',filter:'blur(2px)'}}/>
        ...
        <div style={{width:'100%',maxWidth:'400px',animation:'fadeUp 0.6s ease'}}>
          ...
          <div style={{textAlign:'center',marginBottom:'32px'}}>
            <Link href="/" style={{textDecoration:'none'}}>
              <div style={{fontSize:'48px',lineHeight:1,marginBottom:'8px',filter:'drop-shadow(0 0 20px rgba(255,60,0,0.4))'}}>⛩️</div>
              <div style={{color:'#ff6633',fontWeight:'900',fontSize:'28px',letterSpacing:'2px',textShadow:'0 0 20px rgba(255,60,0,0.5)'}}>生活</div>
              <div style={{color:'rgba(255,255,255,0.4)',fontSize:'12px',letterSpacing:'4px',marginTop:'4px'}}>SEIKATSU</div>
            </Link>
          </div>

          {/* Card */}
          <div style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,100,50,0.15)',borderRadius:'20px',padding:'28px',backdropFilter:'blur(20px)',boxShadow:'0 0 60px rgba(255,40,0,0.06), inset 0 1px 0 rgba(255,255,255,0.05)'}}>
            ...
            <div style={{display:'flex',background:'rgba(255,255,255,0.04)',borderRadius:'12px',padding:'4px',marginBottom:'24px'}}>
              {(['login','signup'] as const).map(m=>(
                <button key={m} onClick={()=>{setMode(m);setMessage('');}} style={{
                  ...
                  background:mode===m?'rgba(255,80,30,0.2)':'transparent',
                  color:mode===m?'#ff6633':'rgba(255,255,255,0.3)',
                  fontSize:'13px',fontWeight:'700',cursor:'pointer',
                  transition:'all 0.2s',letterSpacing:'0.5px',
                  boxShadow:mode===m?'0 0 15px rgba(255,60,0,0.15)':'none',
                }}>
            ...
            <button onClick={handleGoogle} style={{
              ...
              color:'rgba(255,255,255,0.8)',padding:'11px',fontSize:'13px',
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'12px',marginBottom:'16px'}}>
              <div>
                ...
                <input type="email" value={email} onChange={e=>setEmail(e.target.value)}
                  ...
                  style={{width:'100%',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,100,50,0.2)',padding:'11px 14px',fontSize:'14px',outline:'none',borderRadius:'10px',transition:'border-color 0.2s',boxSizing:'border-box'}}
            ...
            {message&&(
              <div style={{fontSize:'12px',padding:'10px 14px',borderRadius:'10px',marginBottom:'14px',textAlign:'center',
                background:message.includes('check')||message.includes('created')?'rgba(0,200,100,0.08)':'rgba(255,60,0,0.08)',
                border:message.includes('check')||message.includes('created')?'1px solid rgba(0,200,100,0.2)':'1px solid rgba(255,60,0,0.2)',
                color:message.includes('check')||message.includes('created')?'#00cc66':'#ff6644',
              }}>{message}</div>
            )}

            <button onClick={handle} disabled={loading||!email||!password} style={{
              width:'100%',
              background:loading||!email||!password?'rgba(255,80,30,0.1)':'rgba(255,80,30,0.25)',
              border:'1px solid rgba(255,80,30,0.4)',
              color:loading||!email||!password?'rgba(255,100,50,0.4)':'#ff6633',
              padding:'12px',fontSize:'14px',fontWeight:'700',
              cursor:loading||!email||!password?'not-allowed':'pointer',
              borderRadius:'12px',letterSpacing:'1px',transition:'all 0.2s',
              boxShadow:loading||!email||!password?'none':'0 0 20px rgba(255,60,0,0.15)',
            }}>
              {loading?'...' : mode==='login'?'SIGN IN':'CREATE ACCOUNT'}
            </button>
          </div>

          <div style={{textAlign:'center',marginTop:'20px',fontSize:'12px',color:'rgba(255,255,255,0.15)'}}>
            <Link href="/" style={{color:'rgba(255,100,50,0.5)',textDecoration:'none'}}>← Back to Seikatsu</Link>
          </div>
